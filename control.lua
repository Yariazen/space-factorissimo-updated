...
local function create_factory_position()
	...
	local surface_name = "Space Factory floor " .. global.next_factory_surface
	...
	if surface == nil then
		if #(game.surfaces) < 256 then
			...
			global.next_factory_surface = 1
			surface_name = "Space Factory floor 1"
...
local SAVE_ITEMS = {}
for _,f in ipairs({"space-factory-3"}) do
	SAVE_ITEMS[f] = {}
	for n = 10,99 do
...
commands.add_command("give-lost-space-factory-buildings", {"command-help-message.give-lost-factory-buildings"}, function(event)
...
local function get_camera_frame(player)
	...
	if not camera_frame then
		camera_frame = frameflow.add{type = "frame", name = "space_factory_camera_frame", style = "captionless_frame"}
...
local function set_camera(player, factory, inside)
	...
	if camera then
		...
		local camera = camera_frame.add{type = "camera", name = "space_factory_camera", position = position, surface_index = surface_index, zoom = zoom}
		camera.style.minimal_width = preview_size
		camera.style.minimal_height = preview_size
	end
